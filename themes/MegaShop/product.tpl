...
<div id="primary_block" class="clearfix">
	...
	<div id="pb-right-column">
		...
		{if isset($images) && count($images) > 3}<a id="view_scroll_left" class="hidden" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}"></a>{/if}
		<div id="thumbs_list">
		...
		{if isset($images) && count($images) > 3}<a id="view_scroll_right" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}"></a>{/if}
		</div>
		{/if}
				...
				<ul>
					{foreach from=$accessories item=accessory name=accessories_list}
						{if ($accessory.allow_oosp || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
							...
							<li class="ajax_block_product product_accessories_description clearfix">
								...
								<h5>-->
									<a class="prod_name" href="{$accessoryLink|escape:'htmlall':'UTF-8'}">{$accessory.name|truncate:23:'...'|escape:'htmlall':'UTF-8'}</a>
									{if $accessory.show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}<span class="price">{if $priceDisplay != 1}{displayWtPrice p=$accessory.price}{else}{displayWtPrice p=$accessory.price_tax_exc}{/if}</span>{/if}
								<!--</h5>-->
								<p class="clearfix" style="margin-top:7px">
