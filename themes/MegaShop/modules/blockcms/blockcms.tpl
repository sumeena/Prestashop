...
{else}
	...
	<div class="block_various_links" id="block_various_links_footer">
		<span class="title_block">{l s='Navigate' mod='blockcms'}</span>
		<a class="toggler"></a>
		<ul>
			{if !$PS_CATALOG_MODE}
			...
			{if !$PS_CATALOG_MODE}
                            {*<li class="item"><a href="{$link->getPageLink('best-sales')}" title="{l s='Top sellers' mod='blockcms'}">{l s='Top sellers' mod='blockcms'}</a></li>*}{/if}
			{if $display_stores_footer}
                            {*<li class="item"><a href="{$link->getPageLink('stores')}" title="{l s='Our stores' mod='blockcms'}">{l s='Our stores' mod='blockcms'}</a></li>*}{/if}
			<li class="item"><a href="{$link->getPageLink($contact_url, true)}" title="{l s='Contact' mod='blockcms'}">{l s='Contact' mod='blockcms'}</a></li>
			...
			{if $display_poweredby}<li class="last_item">{l s='Powered by' mod='blockcms'} <a target="_blank" href="http://www.prestashop.com">PrestaShop</a>&trade;</li>{/if}
                        <li>Copyright &copy; 2011 <br> Cinthe.com</li>
		</ul>
	{$footer_text}
	</div>
	<!-- /MODULE Block footer -->
