	...
	{foreach from=$products item=product name=products}
		<li class="ajax_block_product clearfix">
			...
			<div class="right_block">	
				<a class="prod_name" href="{$product.link|escape:'htmlall':'UTF-8'}" title="{$product.name|escape:'htmlall':'UTF-8'}">{$product.name|escape:'htmlall':'UTF-8'|truncate:29:'...'}</a>
				...
				{if (!$PS_CATALOG_MODE AND ((isset($product.show_price) && $product.show_price) || (isset($product.available_for_order) && $product.available_for_order)))}
					<div class="content_price">
						...
						{if !$priceDisplay}
							{if {convertPrice price=$product.price_without_reduction} > {convertPrice price=$product.price}}
