...
<div id="cart_block" class="block exclusive">
	<h4 class="heading-hightlight">
		<span>{l s='Shopping Cart' mod='blockcart'}</span>
		{if $ajax_allowed}
	...
	{if $discounts|@count > 0}
		...
		{/if}
		<p id="cart-buttons">
			{if $order_process == 'order'}<a href="{$link->getPageLink("$order_process", true)}" class="button_small" title="{l s='Cart' mod='blockcart'}">{l s='My Cart' mod='blockcart'}</a>{/if}
