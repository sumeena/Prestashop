...
<div id="primary_block" class="clearfix">
	...
	<div id="pb-right-column">
		...
		<ul id="usefull_link_block">
			...
			<li><span class="span_link" id="view_full_size">{l s='View full size'}</span></li>
		...
		<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post">
			...
			<div class="price">
				...
				{if $priceDisplay >= 0 && $priceDisplay <= 2}
					...
					{/if}-->
				{/if}
				{if $product->specificPrice AND $product->specificPrice.reduction_type == 'percentage'}<span id="reduction_percent_display">{l s='('} -{$product->specificPrice.reduction*100}%{l s=')'}</span>{/if}
				<span id="reduction_amount_display">{if $product->specificPrice AND $product->specificPrice.reduction_type == 'amount' && $product->specificPrice.reduction|intval !=0}{l s='( -'} {convertPrice price=$product->specificPrice.reduction|floatval}{l s=')'}{/if}</span>
